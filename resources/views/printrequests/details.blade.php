                            ...
                            @if($comment->blocked==0 && is_null($comment->parent_id))
                                ...
                                @if($user->isAdmin())
                                    <p><a class="btn btn-primary"
                                          href={{ route('comments.block', ['requestID' => $requestData->id, 'commentID' => $comment->id]) }}>Block</a>
                                    </p>
                                @endif
                                @foreach ($comments as $subcomment)
                                    @if($comment->id==$subcomment->parent_id && $subcomment->blocked==0)
                                        ...
                                        @if($user->isAdmin())
                                            <p><a class="btn btn-primary"
                                                  href={{ route('comments.block', ['requestID' => $requestData->id, 'commentID' => $subcomment->id]) }}>Block</a>
                                            </p>
                                        @endif
                                    @endif
                                @endforeach
